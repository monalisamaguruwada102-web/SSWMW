...
router.post('/', requireAdmin, async (req, res) => {
    ...
    const id = await runInsert('INSERT INTO users (username, email, password_hash, role) VALUES (?,?,?,?)', [username, email, hash, role]);
...
router.put('/:id', requireAdmin, async (req, res) => {
    ...
    if (password) {
        const hash = bcrypt.hashSync(password, 10);
        await runQuery('UPDATE users SET username=?, email=?, role=?, password_hash=? WHERE id=?',
            [username || user.username, email || user.email, role || user.role, hash, id]);
    } else {
        await runQuery('UPDATE users SET username=?, email=?, role=? WHERE id=?',
