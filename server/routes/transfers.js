...
router.put('/:id/status', authenticateToken, async (req, res) => {
    ...
    const transfer = await getOne('SELECT * FROM transfers WHERE id = ?', [transferId]);
    if (!transfer) return res.status(404).json({ error: 'Transfer not found' });

    // Logical check: if status is 'completed', we should move the stock
    if (status === 'completed' && transfer.status !== 'completed') {
        const items = await getAll('SELECT * FROM transfer_items WHERE transfer_id = ?', [transferId]);
    ...
    await runQuery('UPDATE transfers SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [status, transferId]);
